...
def run_web_scraper():
    """Run a web scraper using Scrapy."""
    process = CrawlerProcess(settings={
        'USER_AGENT': 'Mozilla/5.0 (compatible; BenchmarkBot/1.0)',
        'FEEDS': {
            'scraped_data.json': {
                'format': 'json',
                'encoding': 'utf8',
...
def main():
    ...
    print("Starting web scraping benchmark...")
    ...
    if parse_with_beautifulsoup():
        print("BeautifulSoup parsing successful")
    
    # Generate SBOM
    print("Generating SBOM...")
    sbom = generate_sbom()
    with open('r3-benchmark-sbom.json', 'w') as f:
        json.dump(sbom, f, indent=2)
    print("SBOM generated as r3-benchmark-sbom.json")
